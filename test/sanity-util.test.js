...
describe("`normalizeEntries()`", () => {
  const mockEntries = {
    ...
    "image-48ef6974717f0ff28c9fe64392d487423c5f041b-1000x667-jpg": {
      ...
      path:
        "images/kz6i252u/production/48ef6974717f0ff28c9fe64392d487423c5f041b-1000x667.jpg",
  ...
  test("normalizes entries and assets and resolves references", () => {
    ...
    normalizedEntries.slice(0, 3).forEach(entry => {
      ...
      expect(entry.content_img_path.__metadata.id).toBe(
        "image-48ef6974717f0ff28c9fe64392d487423c5f041b-1000x667-jpg"
      );
      expect(entry.content_img_path.__metadata.source).toBe(
        "sourcebit-source-sanity"
      );
      expect(entry.content_img_path.__metadata.modelName).toBe("__asset");
      expect(entry.content_img_path.__metadata.projectId).toBe(
        mockOptions.projectId
      );
      expect(entry.content_img_path.__metadata.projectEnvironment).toBe(
        mockOptions.dataset
      );
      expect(entry.content_img_path.__metadata.createdAt).toBe(
        "2020-01-29T14:00:00Z"
      );
      expect(entry.content_img_path.__metadata.updatedAt).toBe(
        "2020-01-29T15:00:00Z"
      );
    });

    // Checking that the `foxVillageRef` reference was resolved for all entries
    // expect for entry #2, because it would be a self-reference.
    expect(normalizedEntries[0].foxVillageRef.__metadata.id).toBe(
      normalizedEntries[1].__metadata.id
    );
    expect(normalizedEntries[0].foxVillageRef.title).toBe(
      normalizedEntries[1].title
    );
    expect(normalizedEntries[1].foxVillageRef._type).toBe("reference");
    expect(normalizedEntries[1].foxVillageRef._ref).toBe(
      normalizedEntries[1].__metadata.id
    );
    expect(normalizedEntries[2].foxVillageRef.__metadata.id).toBe(
      normalizedEntries[1].__metadata.id
    );
    expect(normalizedEntries[2].foxVillageRef.title).toBe(
      normalizedEntries[1].title
    );
    ...
    expect(normalizedEntries[0].content_img_path.myCustomField).toBe(
      "my custom value"
    );
    expect(
      normalizedEntries[1].content_img_path.myCustomField
    ).not.toBeDefined();
    expect(
      normalizedEntries[2].content_img_path.myCustomField
    ).not.toBeDefined();
  });
});
...
describe("`parseEntryId()`", () => {
  test("returns an object with a `canonicalId  property, indicating the root/canonical ID of the entry", () => {
    ...
    const op3 = sanityUtil.parseEntryId(
      "icb22HOpscEJNDFs0yLkbZ.drafts.whatever"
    );
